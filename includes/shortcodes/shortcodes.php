 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_logo_shortcode' ) ) {

	function pdwp_logo_shortcode( $atts ) {
...
add_shortcode( 'pdwp_logo', 'pdwp_logo_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_nav_shortcode' ) ) {

	function pdwp_nav_shortcode( $atts ) {
...
add_shortcode( 'pdwp_nav', 'pdwp_nav_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_date_shortcode' ) ) {

	function pdwp_date_shortcode( $atts ) {
...
add_shortcode( 'pdwp_date', 'pdwp_date_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_search_shortcode' ) ) {

	function pdwp_search_shortcode( $atts ) {

		// Extract attributes
		extract( shortcode_atts( array(
			'width' 		=> '',
			'height' 		=> '',
			'placeholder' 	=> esc_html__( 'Search', 'relicwp-helper' ),
		...
		$html = '<form method="get" class="pdwp-searchform" id="searchform" action="'. esc_url( home_url( '/' ) ) .'"'. $style .'>';
...
add_shortcode( 'pdwp_search', 'pdwp_search_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_site_url_shortcode' ) ) {

	function pdwp_site_url_shortcode( $atts ) {
...
add_shortcode( 'pdwp_site_url', 'pdwp_site_url_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_login_shortcode' ) ) {

	function pdwp_login_shortcode( $atts ) {

		extract( shortcode_atts( array(
			'custom_url' 		=> '',
			'login_text' 		=> esc_html__( 'Login', 'relicwp-helper' ),
			'logout_text' 		=> esc_html__( 'Log Out', 'relicwp-helper' ),
			'target' 			=> 'self',
			'logout_redirect' 	=> '',
		), $atts ) );
		...
		if ( is_user_logged_in() ) {
			return '<a href="'. esc_url( $logout_url ) .'" title="'. esc_attr( $logout_text ) .'" class="pdwp-logout">'. strip_tags( $logout_text ) .'</a>';
		}

		// Logged out link
		else {
			return '<a href="'. esc_url( $login_url ) .'" title="'. esc_attr( $login_text ) .'" class="pdwp-login" target="_'. esc_attr( $target ) .'">'. strip_tags( $login_text ) .'</a>';
		}

	}

}
add_shortcode( 'pdwp_login', 'pdwp_login_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_current_user_shortcode' ) ) {

	function pdwp_current_user_shortcode( $atts ) {

		extract( shortcode_atts( array(
			'text' 			=> esc_html__( 'Welcome back', 'relicwp-helper' ),
...
add_shortcode( 'pdwp_current_user', 'pdwp_current_user_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_woo_fragments' ) ) {

	function pdwp_woo_fragments( $fragments ) {
		...
		$fragments['.pdwp-woo-total'] 			= '<span class="pdwp-woo-total">' . WC()->cart->get_total() . '</span>';
	    $fragments['.pdwp-woo-cart-count'] 		= '<span class="pdwp-woo-cart-count">' . WC()->cart->get_cart_contents_count() . '</span>';
	    return $fragments;
	}

}
add_filter( 'woocommerce_add_to_cart_fragments', 'pdwp_woo_fragments', 10, 1 );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_woo_cart_icon_shortcode' ) ) {

	function pdwp_woo_cart_icon_shortcode( $atts ) {
		...
		if ( class_exists( 'Elementor\Plugin' )
			...
			return esc_html__( 'This shortcode only works in front end', 'relicwp-helper' );
		}
		...
		if ( ! empty( $color )
			...
			// Add style
			if ( ! empty( $css ) ) {
				echo "<style type=\"text/css\">\n" . wp_strip_all_tags( pdwp_minify_css( $css ) ) . "\n</style>";
			}
...
add_shortcode( 'pdwp_woo_cart', 'pdwp_woo_cart_icon_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_woo_total_cart_shortcode' ) ) {

	function pdwp_woo_total_cart_shortcode() {
		...
		$html  = '<span class="pdwp-woo-total">';
...
add_shortcode( 'pdwp_woo_total_cart', 'pdwp_woo_total_cart_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_woo_cart_items_shortcode' ) ) {

	function pdwp_woo_cart_items_shortcode() {
		...
		$html  = '<span class="pdwp-woo-cart-count">';
...
add_shortcode( 'pdwp_woo_cart_items', 'pdwp_woo_cart_items_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_woo_free_shipping_left' ) ) {

	function pdwp_woo_free_shipping_left( $content, $content_reached, $multiply_by = 1 ) {
		...
		if ( empty( $content ) ) {
			$content = esc_html__( 'Buy for %left_to_free% more and get free shipping', 'relicwp-helper' );
		}

		if ( empty( $content_reached ) ) {
			$content_reached = esc_html__( 'You have Free delivery!', 'relicwp-helper' );
		}
		...
		if ( 0 != $min_free_shipping_amount ) {
			if ( isset( WC()->cart->cart_contents_total ) ) {
				...
				if ( $total >= $min_free_shipping_amount ) {
					...
					$content = str_replace( '%left_to_free%',             '<span class="pdwp-woo-left-to-free">'. wc_price( ( $min_free_shipping_amount - $total ) * $multiply_by ) .'</span>', $content );
					$content = str_replace( '%free_shipping_min_amount%', '<span class="pdwp-woo-left-to-free">'. wc_price( ( $min_free_shipping_amount )          * $multiply_by ) .'</span>', $content );
					return $content;
				}
			}
		}
...
if ( ! function_exists( 'pdwp_woo_free_shipping_left_shortcode' ) ) {

	function pdwp_woo_free_shipping_left_shortcode( $atts, $content ) {
        ...
        return pdwp_woo_free_shipping_left( "<span class='pdwp-woo-free-shipping' data-content='$x' data-reach='$content_reached'>". $content .'</span>', '<span class="pdwp-woo-free-shipping">'. $content_reached .'</span>', $multiply_by );

	}

}
add_shortcode( 'pdwp_woo_free_shipping_left', 'pdwp_woo_free_shipping_left_shortcode' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'update_pdwp_woo_free_shipping_left_shortcode' ) ) {

	function update_pdwp_woo_free_shipping_left_shortcode() {
	    $atts = array();
	    ...
	    if ( ( isset( $_POST['content'] )
	        ...
	        $atts['content_reached'] 	= wp_kses_post($_POST['content_rech_data']);
	        $content 					= str_replace( '+', '%', wp_kses_post($_POST['content']) );
	        $atts['content'] 			= $content;
	        $returnShortCodeValue 		= pdwp_woo_free_shipping_left_shortcode( $atts, '' );
...
add_action( 'wp_ajax_update_pdwp_woo_free_shipping_left_shortcode', 'update_pdwp_woo_free_shipping_left_shortcode' );
add_action( 'wp_ajax_nopriv_update_pdwp_woo_free_shipping_left_shortcode', 'update_pdwp_woo_free_shipping_left_shortcode' );
 ...
 * @since  1.0.0
 */
function pdwp_woo_free_shipping_left_script() {
	wp_register_script( 'owp-free-shipping', plugins_url( '/js/shortcode.min.js', __FILE__ ), false, true );
}
add_action( 'wp_enqueue_scripts', 'pdwp_woo_free_shipping_left_script' );
 ...
 * @since  1.0.0
 */
if ( ! function_exists( 'pdwp_breadcrumb_shortcode' ) ) {

	function pdwp_breadcrumb_shortcode( $atts ) {
		...
		// Return if PdWP_Breadcrumb_Trail doesn't exist
		if ( ! class_exists( 'PdWP_Breadcrumb_Trail' ) ) {
			return;
		}
		...
		if ( ! empty( $color ) || ! empty( $hover_color ) ) {
			...
			if ( ! empty( $color ) ) {
				$css .= '.pdwp-breadcrumb .site-breadcrumbs, .pdwp-breadcrumb .site-breadcrumbs a {color:'. $color .';}';
			}

			if ( ! empty( $hover_color ) ) {
				$css .= '.pdwp-breadcrumb .site-breadcrumbs a:hover {color:'. $hover_color .';}';
			}
		...
		if ( function_exists( 'yoast_breadcrumb' ) && current_theme_supports( 'yoast-seo-breadcrumbs' ) ) {
			...
			if ( $breadcrumbs_position = get_theme_mod( 'pdvs_breadcrumbs_position' ) ) {
		...
		if ( ! is_object( $breadcrumb ) ) {
			$breadcrumb = new PdWP_Breadcrumb_Trail( $args );
		}

		return '<span class="pdwp-breadcrumb'. $class .'">'. $breadcrumb->get_trail() .'</span>';

	}

}
add_shortcode( 'pdwp_breadcrumb', 'pdwp_breadcrumb_shortcode' );