...
if ( ! class_exists( 'RelicWP_Helper_Facebook_Widget' ) ) {
	class RelicWP_Helper_Facebook_Widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				'pdvs_facebook',
				esc_html__( '&raquo; Facebook Like Box', 'relicwp-helper' ),
				array(
					'classname'   => 'widget_facebook_likebox',
					'description' => esc_html__( 'Display a Facebook Like Box to connect visitors to your Facebook Page.', 'relicwp-helper' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( empty( $like_args['href'] ) || ! $this->is_valid_facebook_url( $like_args['href'] ) ) {
				if ( current_user_can('edit_theme_options') ) {
					echo $before_widget;
					echo '<p>' . sprintf( esc_html__( 'It looks like your Facebook URL is incorrectly configured. Please check it in your <a href="%s">widget settings</a>.', 'relicwp-helper' ), esc_url( admin_url( 'widgets.php' ) ) ) . '</p>';
					echo $after_widget;
				}
			...
			$like_bg_colour          = apply_filters( 'pdvs_fb_likebox_bg', ( 'dark' == $like_args['colorscheme'] ? '#000' : '#fff' ), $like_args['colorscheme'] );
			...
			echo $args['before_widget'];
				...
				if ( ! empty( $title ) ) :
					...
					echo apply_filters( 'pdvs_facebook_likebox_title', $likebox_widget_title, $title, $page_url );
			...
			do_action( 'pdvs_stats_extra', 'widget', 'facebook-likebox' );
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'href' ) ); ?>"><?php esc_html_e( 'Facebook Page URL', 'relicwp-helper' ); ?></label>
				...
				<br />
				<small><?php esc_html_e( 'The Like Box only works with ', 'relicwp-helper' ); ?><a href="http://www.facebook.com/help/?faq=174987089221178" target="_blank"><?php esc_html_e( 'Facebook Pages', 'relicwp-helper' ); ?></a></small>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'width' ) ); ?>"><?php esc_html_e( 'Width', 'relicwp-helper' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'height' ) ); ?>"><?php esc_html_e( 'Height', 'relicwp-helper' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'colorscheme' ) ); ?>"><?php esc_html_e( 'Color Scheme', 'relicwp-helper' ); ?></label>
				<select name="<?php echo esc_attr( $this->get_field_name( 'colorscheme' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'colorscheme' ) ); ?>">
					<option value="light" <?php selected( $like_args['colorscheme'], 'light' ); ?>><?php esc_html_e( 'Light', 'relicwp-helper' ); ?></option>
					<option value="dark" <?php selected( $like_args['colorscheme'], 'dark' ); ?>><?php esc_html_e( 'Dark', 'relicwp-helper' ); ?></option>
				</select>
			</p>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_faces' ) ); ?>">
					...
					<?php esc_html_e( 'Show Faces', 'relicwp-helper' ); ?>
					<br />
					<small><?php esc_html_e( 'Show profile photos in the plugin.', 'relicwp-helper' ); ?></small>
				</label>
			</p>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'stream' ) ); ?>">
					...
					<?php esc_html_e( 'Show Stream', 'relicwp-helper' ); ?>
					<br />
					<small><?php esc_html_e( 'Show the profile stream for the public profile.', 'relicwp-helper' ); ?></small>
				</label>
			</p>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'show_border' ) ); ?>">
					...
					<?php esc_html_e( 'Show Border', 'relicwp-helper' ); ?>
					<br />
					<small><?php esc_html_e( 'Show a border around the plugin.', 'relicwp-helper' ); ?></small>
				</label>
			</p>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'force_wall' ) ); ?>">
					...
					<?php esc_html_e( 'Show Wall', 'relicwp-helper' ); ?>
					<br />
					<small><?php esc_html_e( 'Show the wall for a Places page rather than friend activity.', 'relicwp-helper' ); ?></small>
				</label>
			</p>
		...
		public function get_default_args() {
			...
			return apply_filters( 'pdvs_facebook_likebox_defaults', $defaults );
		}
...
register_widget( 'RelicWP_Helper_Facebook_Widget' );