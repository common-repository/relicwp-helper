...
if ( ! class_exists( 'RelicWP_Helper_Video_Widget' ) ) {
	class RelicWP_Helper_Video_Widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				'pdvs_video',
				esc_html__( '&raquo; Video', 'relicwp-helper' ),
				array(
					'classname'   => 'widget-pdwp-video video-widget',
					'description' => esc_html__( 'Easily to display any type of video.', 'relicwp-helper' ),
		...
		public function widget( $args, $instance ) {
			...
			echo $args['before_widget'];
				...
				if ( $video_url )  {
					...
					esc_html_e( 'You forgot to enter a video URL.', 'relicwp-helper' );
				}

				// Show video description if field isn't empty
				if ( $description ) {
					echo '<div class="pdwp-video-widget-description">'. do_shortcode( $description ) .'</div>';
				}
		...
		public function form( $instance ) {
			...
			$instance = wp_parse_args((array) $instance, array(
				'title'             => esc_attr__( 'Video', 'relicwp-helper' ),
				'video_url'         => '',
				'video_description' => '',
			) ); ?>
			...
			<p>
				...
				<?php esc_html_e( 'Video URL ', 'relicwp-helper' ); ?></label>
				...
				<span style="display:block;padding:5px 0" class="description"><?php esc_html_e( 'Enter in a video URL that is compatible with WordPress\'s built-in oEmbed feature.', 'relicwp-helper' ); ?> <a href="http://codex.wordpress.org/Embeds" target="_blank"><?php esc_html_e( 'Learn More', 'relicwp-helper' ); ?></a></span>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'video_description' ) ); ?>"><?php esc_html_e( 'Description', 'relicwp-helper' ); ?></label>
...
register_widget( 'RelicWP_Helper_Video_Widget' );