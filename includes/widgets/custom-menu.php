...
if ( ! class_exists( 'RelicWP_Helper_Custom_Menu_Widget' ) ) {
	class RelicWP_Helper_Custom_Menu_Widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				'pdvs_custom_menu',
				esc_html__( '&raquo; Custom Menu', 'relicwp-helper' ),
				array(
					'classname'   => 'widget-pdwp-custom-menu custom-menu-widget',
					'description' => esc_html__( 'Displays custom menu.', 'relicwp-helper' ),
		...
		public function widget( $args, $instance ) {
			...
			echo $args['before_widget'];
				...
				if ( $nav_padding && '8px 0' != $nav_padding
					...
					if ( $nav_padding && '8px 0' != $nav_padding
						...
						echo '.custom-menu-widget .' . esc_attr( $this->id ) . '.pdwp-custom-menu > ul.click-menu .open-this{';
							if ( $nav_link_color && '#555' != $nav_link_color ) {
								echo 'color:' . esc_attr( $nav_link_color ) . ';';
							}
							if ( $font_size && '13' != $font_size ) {
					...
					if ( $nav_link_hover_color && '#333' != $nav_link_hover_color ) {
						...
						echo '}';
						echo '.custom-menu-widget .' . esc_attr( $this->id ) . '.pdwp-custom-menu > ul.click-menu .open-this:hover{';
							echo 'color:' . esc_attr( $nav_link_hover_color ) . ';';
						echo '}';
					}
				...
				$nav_menu_args = array(
					...
					'walker'        => new RelicWP_Helper_Nav_Walker(),
				);

				// Add classes
				$classes 	= array( 'pdwp-custom-menu', 'clr' );
			...
			if ( 'click' == $dropdown ) { ?>
				<script type="text/javascript">
					( function( $ ) {
						$( '.<?php echo esc_attr( $this->id ); ?>.pdwp-custom-menu.dropdown-click ul.dropdown-menu' ).each( function() {
					        ...
					        target.on( 'click', function() {
								...
								// Target
								if ( $( this ).closest( '.<?php echo esc_attr( $this->id ); ?>.pdwp-custom-menu.dropdown-click' ).hasClass( 'click-link' ) ) {
		...
		public function form( $instance ) {
			...
			<p class="nav-menu-widget-no-menus-message" <?php if ( ! empty( $menus ) ) { echo ' style="display:none" '; } ?>>
				...
				?>
				<?php echo sprintf( esc_html__( 'No menus have been created yet. <a href="%s">Create some</a>.', 'relicwp-helper' ), esc_attr( $url ) ); ?>
			</p>
			<div class="nav-menu-widget-form-controls" <?php if ( empty( $menus ) ) { echo ' style="display:none" '; } ?>>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>"><?php esc_html_e( 'Select Menu:', 'relicwp-helper' ); ?></label>
					<select class="widget-select widefat" id="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'nav_menu' ) ); ?>">
						<option value="0"><?php esc_html_e( '&mdash; Select &mdash;', 'relicwp-helper' ); ?></option>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id('position') ); ?>"><?php esc_html_e( 'Position:', 'relicwp-helper' ); ?></label>
					<select class="widget-select widefat" name="<?php echo esc_attr( $this->get_field_name('position') ); ?>" id="<?php echo esc_attr( $this->get_field_id('position') ); ?>">
						<option value="left" <?php selected( $instance['position'], 'left' ) ?>><?php esc_html_e( 'Left', 'relicwp-helper' ); ?></option>
						<option value="right" <?php selected( $instance['position'], 'right' ) ?>><?php esc_html_e( 'Right', 'relicwp-helper' ); ?></option>
						<option value="center" <?php selected( $instance['position'], 'center' ) ?>><?php esc_html_e( 'Center', 'relicwp-helper' ); ?></option>
					</select>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id('dropdown') ); ?>"><?php esc_html_e( 'Dropdown Style:', 'relicwp-helper' ); ?></label>
					<select class="widget-select widefat" name="<?php echo esc_attr( $this->get_field_name('dropdown') ); ?>" id="<?php echo esc_attr( $this->get_field_id('dropdown') ); ?>">
						<option value="hover" <?php selected( $instance['dropdown'], 'hover' ) ?>><?php esc_html_e( 'Hover', 'relicwp-helper' ); ?></option>
						<option value="click" <?php selected( $instance['dropdown'], 'click' ) ?>><?php esc_html_e( 'Click', 'relicwp-helper' ); ?></option>
					</select>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id('target') ); ?>"><?php esc_html_e( 'Target:', 'relicwp-helper' ); ?></label>
					<select class="widget-select widefat" name="<?php echo esc_attr( $this->get_field_name('target') ); ?>" id="<?php echo esc_attr( $this->get_field_id('target') ); ?>">
						<option value="icon" <?php selected( $instance['target'], 'icon' ) ?>><?php esc_html_e( 'Icon', 'relicwp-helper' ); ?></option>
						<option value="link" <?php selected( $instance['target'], 'link' ) ?>><?php esc_html_e( 'Link', 'relicwp-helper' ); ?></option>
					</select>
					<small><?php esc_html_e( 'If the Click dropdown style is selected', 'relicwp-helper' ); ?></small>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'nav_padding' ) ); ?>"><?php esc_html_e( 'Menu Padding:', 'relicwp-helper' ); ?></label>
					...
					<small style="color: #777;"><?php esc_html_e( 'top left bottom right, eg: 15px 8px 15px 25px', 'relicwp-helper' ); ?></small>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'nav_link_color' ) ); ?>"><?php esc_html_e( 'Menu Link Color:', 'relicwp-helper' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'nav_link_hover_color' ) ); ?>"><?php esc_html_e( 'Menu Link Hover Color:', 'relicwp-helper' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'font_size' ) ); ?>"><?php esc_html_e( 'Font Size (px):', 'relicwp-helper' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'line_height' ) ); ?>"><?php esc_html_e( 'Line Height (px):', 'relicwp-helper' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'letter_spacing' ) ); ?>"><?php esc_html_e( 'Letter Spacing (px):', 'relicwp-helper' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id('text_transform') ); ?>"><?php esc_html_e( 'Text Transform:', 'relicwp-helper' ); ?></label>
					<select class="widget-select widefat" name="<?php echo esc_attr( $this->get_field_name('text_transform') ); ?>" id="<?php echo esc_attr( $this->get_field_id('position') ); ?>">
						<option value="default" <?php selected( $instance['text_transform'], 'default' ) ?>><?php esc_html_e( 'Default', 'relicwp-helper' ); ?></option>
						<option value="capitalize" <?php selected( $instance['text_transform'], 'capitalize' ) ?>><?php esc_html_e( 'Capitalize', 'relicwp-helper' ); ?></option>
						<option value="lowercase" <?php selected( $instance['text_transform'], 'lowercase' ) ?>><?php esc_html_e( 'Lowercase', 'relicwp-helper' ); ?></option>
						<option value="uppercase" <?php selected( $instance['text_transform'], 'uppercase' ) ?>><?php esc_html_e( 'Uppercase', 'relicwp-helper' ); ?></option>
					</select>
				</p>
...
register_widget( 'RelicWP_Helper_Custom_Menu_Widget' );