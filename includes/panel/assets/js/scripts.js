...
$j(document).ready(function(e) {
	...
	(function () {
		...
		var $items 				= $j( '.pdwp-modules .modules-inner' ).find( '.column-wrap' ),
		...
		$j( '.pdwp-modules .btn-switcher li a' ).on( 'click', function( event ) {
		...
		$j( document ).on( 'click', '#owp-switch-all', function() {
			if ( $j( this ).is( ':checked' ) ) {
			    $j( this ).closest( '.pdwp-modules' ).find( 'input.owp-checkbox' ).prop( 'checked', true );
			} else {
			    $j( this ).closest( '.pdwp-modules' ).find( 'input.owp-checkbox' ).prop( 'checked', false );
			}
		} );
	}());
