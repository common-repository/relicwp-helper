<?php
final class RelicWP_Icon_Selector {

	const VERSION = '1.0.0';

	/**
	 * RelicWP_Icon_Selector singleton
	 * 
	 */
	...
	protected function __construct( $args = array() ) {
		$defaults = array(
			'dir' => RTHP_PATH .'includes/menu-icons/includes/library/icon-selector',
			'url' => RTHP_URL .'includes/menu-icons/includes/library/icon-selector',
		);
	...
	public function init() {
		 ...
		 * 
		 */
		do_action( 'relicwp_icon_picker_init', $this );
	}
	...
	protected function init_registry() {
		...
		$this->registry = RelicWP_Icon_Picker_Types_Registry::instance();
		 ...
		 * 
		 */
		do_action( 'relicwp_icon_picker_types_registry_ready', $this );
	}
	...
	protected function register_default_types() {
		...
		RelicWP_Icon_Picker_Fontpack::instance();

		/**
		 * Allow themes/plugins to disable one or more default types
		 * 
		 */
		$default_types = array_filter( (array) apply_filters( 'relicwp_icon_picker_default_types', $this->default_types ) );
		...
		foreach ( $default_types as $filename => $class_suffix ) {
			$class_name = "RelicWP_Icon_Picker_Type_{$class_suffix}";
	...
	protected function init_loader() {
		...
		$this->loader = RelicWP_Icon_Picker_Loader::instance();

		/**
		 * Fires when Icon Picker's Registry is ready and the default types are registered.
		 * 
		 */
		do_action( 'relicwp_icon_picker_loader_ready', $this );
	}
...
add_action( 'wp_loaded', array( 'RelicWP_Icon_Selector', 'instance' ), 7 );
