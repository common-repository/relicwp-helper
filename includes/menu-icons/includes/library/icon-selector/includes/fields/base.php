 ...
 *
 */
function relicwp_icon_picker_get_icon_url( $type, $id, $size = 'thumbnail' ) {
	$url = '';
 ...
 *
 */
function relicwp_icon_picker_field( $args, $echo = true ) {
	$defaults = array(
		'id'    => '',
		'name'  => '',
		'value' => array(
		...
		'select' => sprintf( '<a class="ipf-select">%s</a>', esc_html__( 'Select Icon', 'relicwp-helper' ) ),
		'remove' => sprintf( '<a class="ipf-remove button hidden">%s</a>', esc_html__( 'Remove', 'relicwp-helper' ) ),
	);
	...
	$field .= sprintf(
		...
		esc_attr( relicwp_icon_picker_get_icon_url( $args['value']['type'], $args['value']['icon'] ) )
	);
	$field .= '</div>';

	if ( $echo ) {
		echo wp_kses_post( $field ); // xss ok
	} else {
		return $field;
	}
}
