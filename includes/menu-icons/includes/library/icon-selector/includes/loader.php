...
final class RelicWP_Icon_Picker_Loader {

	/**
	 * RelicWP_Icon_Picker_Loader singleton
	 *
	 */
	...
	protected function __construct() {
		 ...
		 *
		 */
		do_action( 'relicwp_icon_picker_loader_init', $this );
	}
	...
	public function load() {
		if ( ! is_admin() ) {
			_doing_it_wrong(
				...
				esc_html( RelicWP_Icon_Selector::VERSION )
			);

			return;
		}

		if ( ! did_action( 'relicwp_icon_picker_loader_init' ) ) {
			_doing_it_wrong(
				__METHOD__,
				sprintf(
					'It should not be called until the %s hook.',
					'<code>relicwp_icon_picker_loader_init</code>'
				),
				esc_html( RelicWP_Icon_Selector::VERSION )
			);
	...
	public function _media_view_strings( $strings ) {
		$strings['iconPicker'] = array(
			'frameTitle' => __( 'Icon Picker', 'relicwp-helper' ),
			'allFilter'  => __( 'All', 'relicwp-helper' ),
			'selectIcon' => __( 'Select Icon', 'relicwp-helper' ),
		);
	...
	public function _enqueue_assets() {
		 ...
		 *
		 */
		do_action( 'relicwp_icon_picker_admin_loaded', $icon_picker );
	}
	...
	public function _media_templates() {
		 ...
		 *
		 */
		do_action( 'relicwp_icon_picker_print_media_templates', $icon_picker );
	}
