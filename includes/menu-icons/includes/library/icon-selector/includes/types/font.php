 ...
 *
 */
abstract class RelicWP_Icon_Picker_Type_Font extends RelicWP_Icon_Picker_Type {
	...
	public function get_stylesheet_uri() {
		...
		if ( 'font-awesome' != $this->stylesheet_id
			...
			$stylesheet_uri = sprintf(
				...
				RelicWP_Icon_Selector::instance()->url,
				$this->stylesheet_id,
		...
		if ( class_exists( 'PDWP_Theme_Class' ) ) {
			if ( 'font-awesome' == $this->stylesheet_id ) {
				$stylesheet_uri = PDWP_THEME_URI .'/assets/fonts/fontawesome/css/all.min.css';
			}

			if ( 'simple-line-icons' == $this->stylesheet_id ) {
				$stylesheet_uri = PDWP_CSS_DIR_URI .'third/simple-line-icons.min.css';
			}
		}
		...
		$stylesheet_uri = apply_filters(
			'relicwp_icon_picker_icon_type_stylesheet_uri',
			$stylesheet_uri,
			$this->id,
			$this
		);
	...
	public function __construct( array $args = array() ) {
		...
		add_action( 'relicwp_icon_picker_loader_init', array( $this, 'register_assets' ) );
	}
	...
	public function get_templates() {
		 ...
		 *
		 */
		$templates = apply_filters( 'relicwp_icon_picker_font_media_templates', $templates );
