...
final class RelicWP_Icon_Picker_Fontpack {

	/**
	 * RelicWP_Icon_Picker_Fontpack singleton
	 *
	 */
	...
	protected function __construct() {
		 ...
		 *
		 */
		$this->dir = apply_filters( 'relicwp_icon_picker_fontpacks_dir_path', WP_CONTENT_DIR . '/fontpacks' );
		 ...
		 *
		 */
		$this->url = apply_filters( 'relicwp_icon_picker_fontpacks_dir_url', WP_CONTENT_URL . '/fontpacks' );

		$this->messages = array(
			'no_config'    => __( 'Icon Picker: %1$s was not found in %2$s.', 'relicwp-helper' ),
			'config_error' => __( 'Icon Picker: %s contains an error or more.', 'relicwp-helper' ),
			'invalid'      => __( 'Icon Picker: %1$s is not set or invalid in %2$s.', 'relicwp-helper' ),
			'duplicate'    => __( 'Icon Picker: %1$s is already registered. Please check your font pack config file: %2$s.', 'relicwp-helper' ),
		);
	...
	protected function register_packs() {
		...
		$icon_picker = RelicWP_Icon_Selector::instance();
		require_once "{$icon_picker->dir}/includes/types/fontello.php";

		foreach ( $this->packs as $pack_data ) {
			$icon_picker->registry->add( new RelicWP_Icon_Picker_Type_Fontello( $pack_data ) );
		}
	}
	...
	protected function get_pack_data( DirectoryIterator $pack_dir ) {
		...
		$pack_data = array(
			'id'             => "pack-{$config['name']}",
			'name'           => sprintf( __( 'Pack: %s', 'relicwp-helper' ), $config['name'] ),
