 ...
 * Nav menu admin
 */
final class RelicWP_Menu_Icons_Picker {
	...
	public static function init() {
		...
		add_filter( 'relicwp_icon_picker_type_props', array( __CLASS__, '_add_extra_type_props_data' ), 10, 3 );
	}
	...
	public static function _load_nav_menus() {
		RelicWP_Icon_Selector::instance()->load();
	...
	protected static function _get_menu_item_fields( $meta ) {
		$fields = array_merge(
			...
			RelicWP_Menu_Icons_Settings::get_settings_fields( $meta )
		);
	...
	public static function _fields( $id, $item, $depth, $args ) {
		$input_id      = sprintf( 'pt-icons-%d', $item->ID );
		$input_name    = sprintf( 'pt-icons[%d]', $item->ID );
		$menu_settings = RelicWP_Menu_Icons_Settings::get_menu_settings( RelicWP_Menu_Icons_Settings::get_current_menu_id() );
		$meta          = RelicWP_Menu_Icons_Meta::get( $item->ID, $menu_settings );
		$fields        = self::_get_menu_item_fields( $meta );
		?>
			<div class="field-icon description-wide pt-icons-wrap" data-id="<?php echo json_encode( $item->ID ); ?>">
				<?php
					/**
					 * Add filter to allow to inject HTML before menu icons fields
					 */
					do_action( 'relicwp_menu_icons_before_fields', $item, $depth, $args, $id );
				?>
				<p class="description submitbox">
					<label><?php esc_html_e( 'Icon:', 'relicwp-helper' ) ?></label>
					<?php printf( '<a class="_select">%s</a>', esc_html__( 'Select', 'relicwp-helper' ) ); ?>
					<?php printf( '<a class="_remove submitdelete hidden">%s</a>', esc_html__( 'Remove', 'relicwp-helper' ) ); ?>
				</p>
				<div class="_settings hidden">
					<?php
					foreach ( $fields as $field ) {
						printf(
							'<label>%1$s: <input type="text" name="%2$s" class="_pt-%3$s" value="%4$s" /></label><br />',
					...
					printf( '<input type="hidden" class="_pt-url" value="%s" />', esc_attr( $meta['url'] ) );
					?>
				</div>
				<?php
					/**
					 ...
					 */
					do_action( 'relicwp_menu_icons_after_fields', $item, $depth, $args, $id );
				?>
			</div>
		<?php
	}
	...
	public static function _columns( $columns ) {
		$columns['icon'] = __( 'Icon', 'relicwp-helper' );
	...
	public static function _save( $menu_id, $menu_item_db_id, $menu_item_args ) {
		...
		// Sanitize
		if ( ! empty( $_POST['pt-icons'][ $menu_item_db_id ] ) ) {
			$value = array_map(
				'sanitize_text_field',
				wp_unslash( (array) $_POST['pt-icons'][ $menu_item_db_id ] )
			);
		} else {
			$value = array();
		}

		RelicWP_Menu_Icons_Meta::update( $menu_item_db_id, $value );
	}
	...
	protected static function _print_tempate( $id, $template ) {
		?>
			<script type="text/html" id="<?php echo esc_attr( $id ) ?>">
				<?php echo esc_js($template); // xss ok ?>
			</script>
		<?php
	}
