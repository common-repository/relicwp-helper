 ...
 * @license   GPLv2
 */
final class RelicWP_Menu_Icons {
	 ...
	 * 3. Attach plugin initialization at relicwp_icon_picker_init hook
	 * 
	 */
	public static function _load() {
		...
		self::$data = array(
			'dir'   => RTHP_PATH .'includes/menu-icons/',
			'url'   => RTHP_URL .'includes/menu-icons/',
			'types' => array(),
		);

		// Load Icon Picker.
		if ( ! class_exists( 'RelicWP_Icon_Selector' ) ) {
		...
		RelicWP_Icon_Selector::instance();
		...
		RelicWP_Menu_Icons_Meta::init();

		add_action( 'relicwp_icon_picker_init', array( __CLASS__, '_init' ), 9 );
	}
	...
	public static function _init() {
		...
		self::$data['types'] = apply_filters(
			'relicwp_menu_icons_types',
			RelicWP_Icon_Picker_Types_Registry::instance()->types
		);

		// Nothing to do if there are no icon types registered.
		if ( empty( self::$data['types'] ) ) {
			if ( WP_DEBUG ) {
				trigger_error( esc_html__( 'Menu Icons: No registered icon types found.', 'relicwp-helper' ) );
			}
		...
		RelicWP_Menu_Icons_Settings::init();

		// Load front-end functionalities.
		if ( ! is_admin() ) {
			require_once self::$data['dir'] . '/includes/front.php';
			RelicWP_Menu_Icons_Front_End::init();
		}

		do_action( 'relicwp_menu_icons_loaded' );
	}
	...
	public static function _notice_missing_icon_picker() {
		?>
		<div class="error">
			<p><?php esc_html_e( 'Please activate Icon Picker first.', 'relicwp-helper' ); ?></p>
		</div>
		<?php
	}
}
add_action( 'init', array( 'RelicWP_Menu_Icons', '_load' ) );
