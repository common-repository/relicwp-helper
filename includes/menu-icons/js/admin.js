   ...
   */
		setIcon: function setIcon($el) {
			...
			$el.find('div._settings input').each(function () {
				var $input = $(this),
				    key = $input.attr('class').replace('_pt-', ''),
				    value = $input.val();
   ...
   */
		updateField: function updateField(e) {
			var $el = $(e.currentTarget),
			    ...
			    type = $el.find('input._pt-type').val(),
			    icon = $el.find('input._pt-icon').val(),
			    url = $el.find('input._pt-url').val(),
			    template;
			...
			if (miPicker.templates[type]) {
				...
				template = miPicker.templates[type] = wp.template('pt-icons-item-field-preview-' + iconPicker.types[type].templateId);
			}
...
(function ($) {
  ...
  */
	$('#pt-icons-settings-tabs').on('click', 'a.pt-settings-nav-tab', function (e) {
		var $el = $(this).blur(),
		    $target = $('#' + $el.data('type'));
	...
	}).find('a.pt-settings-nav-tab').first().click();

	// Settings meta box
	$('#pt-icons-settings-save').on('click', function (e) {
		...
		$.ajax({
			...
			data: $('#pt-icons-settings :input').serialize(),
...
var MenuIconsItemPreview = wp.media.View.extend({
	tagName: 'p',
	className: 'pt-preview menu-item attachment-info',
	events: {
	...
	render: function render() {
		var frame = this.controller,
		    ...
		    template = 'pt-icons-item-sidebar-preview-' + iconPicker.types[state.id].templateId + '-';
...
MenuIconsItemSettingField = wp.media.View.extend({
	...
	initialize: function initialize() {
		...
		this.template = wp.media.template('pt-icons-settings-field-' + this.model.get('type'));
		this.model.on('change', this.render, this);
	},
...
var MenuIconsItemSettings = wp.media.view.PriorityList.extend({
	className: 'pt-settings attachment-info',
