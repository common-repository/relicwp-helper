...
if ( ! class_exists( 'RelicWP_Helper_Nav_Walker' ) ) {

	class RelicWP_Helper_Nav_Walker extends Walker_Nav_Menu {
		...
		public function display_element( $element, &$children_elements, $max_depth, $depth=0, $args, &$output ) {
			...
			$header_style = pdwp_header_style();
			...
			if ( ! empty( $children_elements[$element->$id_field] ) && ( $depth == 0 )
				...
				if ( true == get_theme_mod( 'pdvs_menu_arrow_down', true ) ) {
			...
			if ( ! empty( $children_elements[$element->$id_field] ) && ( $depth > 0 ) ) {
				$element->classes[] = 'dropdown';
				if ( true == get_theme_mod( 'pdvs_menu_arrow_side', true ) ) {
					if ( is_rtl() ) {
