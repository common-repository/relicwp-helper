( function() {
	...
	butterbean.views.register_control( 'range', {
		...
		ready: function() {
			...
			jQuery( 'input[type=range]' ).on( 'mousedown', function() {
				...
				range 			= jQuery( this );
				range_input 	= range.parent().children( '.pdwp-range-input' );
			...
			var pdwp_range_input_number_timeout;

			function pdwp_autocorrect_range_input_number( input_number, timeout ) {
				...
				clearTimeout( pdwp_range_input_number_timeout );

				pdwp_range_input_number_timeout = setTimeout( function() {
			...
			jQuery( 'input.pdwp-range-input' ).on( 'change keyup', function() {

				pdwp_autocorrect_range_input_number( jQuery( this ), 1000);

			} ).on( 'focusout', function() {

				pdwp_autocorrect_range_input_number( jQuery( this ), 0);

			} );

			// Handle the reset button
			jQuery( '.pdwp-reset-slider' ).on('click', function() {
	...
	butterbean.views.register_control( 'media', {
		...
		// Adds custom events.
		events : {
			'click .pdwp-add-media' 	  : 'showmodal',
		},
