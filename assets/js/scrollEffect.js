	    ...
	    $j( 'a[href*="#"]:not([href="#"])' ).on( 'click', function() {
	        ...
	        if ( ! $j( this ).hasClass( 'no-effect' )
	        	...
	        	var $href     				= $j( this ).attr( 'href' ),
					...
					$topbarHeight        	= pdwpGetTopbarHeight(),
					$stickyHeaderHeight    	= pdwpGetStickyHeaderHeight(),
				    $scrollPosition;
...
// Admin bar height
function pdwpGetAdminbarHeight() {
	"use strict"
...
// Top bar height
function pdwpGetTopbarHeight() {
	"use strict"

	var $topbarHeight = 0;

	if ( $j( '#top-bar-wrap' ).hasClass( 'pdwp-top-bar-sticky' )
...
// Header height
function pdwpGetStickyHeaderHeight() {
	"use strict"
